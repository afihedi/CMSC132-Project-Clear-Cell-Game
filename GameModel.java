...
public abstract class GameModel {
	...
	public GameModel(int rows, int cols) {
		this.rows = rows;
		this.cols = cols;
		this.board = new BoardCell[this.rows][this.cols]; //declaring the variable
		for(int boardCellRow = 0; boardCellRow < this.rows; boardCellRow++) {
			for(int boardCellCol = 0; boardCellCol < this.cols; boardCellCol++) {
				this.board[boardCellRow][boardCellCol] = BoardCell.EMPTY; 
				//using loops to make each cell empty
			}
		}
	}
